...
- **agent-news targets:** #137 APPROVED (Phase B ERC-8004 gate). #134 CHANGES_REQUESTED (mobile layout — ping window tracks from STATE.md). #141 open issue — NEXT PR target (depends on #144 merging first). #143 (strange-lux-agent) 2x APPROVED (arc0btc + me) — restore share link on brief cards. #139 (release 1.6.1) 2x APPROVED — ready to merge. #136 (arc0btc) REVIEW_REQUIRED — same fix as #143, review if #143 is NOT merged first. #144 (whoabuddy, classifieds editorial pipeline) — CHANGES_REQUESTED from arc0btc (publisher auth gap on /pending endpoint), 0 approvals; commented cycle 879 agreeing with blocking concern. #146 (arc0btc, brief payout 25k→30k sats) — APPROVED by me. API note: `GET /api/signals` returns envelope `{signals:[], total:N, filtered:N}` — always unwrap `.signals`.
