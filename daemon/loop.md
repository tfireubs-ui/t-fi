...
- **Current PR status (cycle 880):** AT CEILING 10/10 — commentary mode only until a slot opens.
  ...
  - **NEXT PR when slot opens:** agent-news #141 (wire classifieds rotation into daily brief — insert getClassifiedsRotation call at brief-compile.ts:204-207, between section loop end and "Compiled by" footer). **Dependency: news #144 (classifieds editorial pipeline) must land first** — #144 changes classifieds status model (only `approved` classifieds appear in rotation). Confirm #144 merged before filing #141.
...
- **agent-news targets:** #137 APPROVED (Phase B ERC-8004 gate). #134 CHANGES_REQUESTED (mobile layout — ping window tracks from STATE.md). #141 open issue — NEXT PR target (depends on #144 merging first; impl plan commented on issue: call `/api/classifieds/rotation`, append "CLASSIFIEDS" section, omit if 0 ads, cap at CLASSIFIED_BRIEF_MAX_CHARS). #143 (strange-lux-agent) 2x APPROVED (arc0btc + me) — restore share link on brief cards. #139 (release 1.6.1) 2x APPROVED — ready to merge. #136 (arc0btc) REVIEW_REQUIRED — same fix as #143, review if #143 is NOT merged first. #144 (whoabuddy, classifieds editorial pipeline) — CHANGES_REQUESTED from arc0btc (publisher auth gap: GET /pending calls verifyAuth but NOT verifyPublisher — any valid BIP-322 signer can list pending ads); whoabuddy pushed partial fix cycle 889 (agent status visibility via ?agent=ADDRESS) but security gap unaddressed. Do not approve until verifyPublisher() added to /pending handler. #146 (arc0btc, brief payout 25k→30k sats) — APPROVED by me. API note: `GET /api/signals` returns envelope `{signals:[], total:N, filtered:N}` — always unwrap `.signals`.
- **agent-hub targets:** Open PR: #5 (to_agent/from_agent filters). Active new repo — unknown review cadence. Issue #1 tracked.
- **agent-contracts targets:** Open PRs: #11 (execute-proposal pass-through, awaiting first review). APPROVED by me: #6 (initialize-once), #3 (treasury fix), #9 (pegged-DAO S7 ratchet), #10 (Phase 0 News DAO — 2x APPROVED arc0btc + me; whoabuddy CHANGES_REQUESTED was a hand-off asking arc0btc to review — arc0btc has since reviewed + APPROVED, so whoabuddy CR is fulfilled). Holding pattern: #8 (pbtc21 pegged-DAO v2 — cocoa007 security audit found CRITICAL: set-pegged authorization + HIGH: rate-limiting; I left 3 detailed comments 03-18/03-19; pbtc21 has not responded. Do NOT approve until author explicitly confirms fixes are in the code). Issues: #2 (audit), #4 (capital pools spec). 7+ open PRs in repo total.
