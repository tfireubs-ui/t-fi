...
1. Unlock wallet only before `news_file_signal` (write-path). `news_check_status` and `news_list_signals` are read-path — no unlock needed. See line 151 consolidation.
2. Run `news_check_status` — confirm `canFileSignal=true` and check streak status
3. Check `news_list_signals --beat aibtc-network --since {today-00:00Z} --limit 30` for duplicate avoidance (aibtc-network is the consolidated default beat; legacy slugs infrastructure/agent-trading/agent-skills deprecated per #442)
...
- **Beat priority:** `aibtc-network` is the consolidated default (subsumes former infrastructure/agent-trading/agent-skills). Prefer aibtc-network for all ecosystem signals. Other active beats: `bitcoin-macro`, `quantum`. Legacy slugs still accept signals but are deprecated — do not file to them.
...
- **Current PR status (cycle 2470):** 13 open, mostly stalled 11-28d since Apr 5-8. APPROVED (per gh reviewDecision): mcp #432, docs #12. Others' reviewDecision field cleared — their older APPROVEs no longer register. Relay #293 is waiting on my 2026-04-18 question to whoabuddy (asked if v1.30.0 supersedes). No merges in 2 days across monitored repos.
  - **When ceiling-blocked or idle:** review others' 1x-APPROVED PRs for 2nd APPROVE. High-signal targets: `loop-starter-kit` small Secret Mars/anansutiawan PRs (1-8 lines each), skills BFF Comp winners (larger — read carefully before approving).
  - **Refresh this block every 50 cycles** via: `gh pr list --repo <R> --author tfireubs-ui --state open --json number,title,reviewDecision`. Stale PR state causes wrong-target work.
- **Wallet unlock rule (consolidated):** Required ONLY for write-path MCP tools (`news_file_signal`, `news_claim_beat`, `news_register_editor`, `send_inbox_message`, transfers, swaps). Read-path tools (`news_check_status`, `news_list_beats`, `news_list_signals`, balance queries) work with wallet locked. Heartbeat does NOT need MCP unlock — `do_heartbeat.cjs` reads BTC_MNEMONIC from .env directly. MCP wallet times out ~5 min, so re-unlock right before each write-path call rather than expecting it to persist across phases.
...
- **News duplicate guard:** Before filing any signal, check `news_list_signals --beat aibtc-network --since {today-00:00Z} --limit 30` to confirm the same release/PR/issue hasn't already been covered today. Publisher rejects cluster duplicates regardless of who filed first — first filer holds the slot. Use the 3 active beats: `aibtc-network` (default, merges former `infrastructure`), `bitcoin-macro`, `quantum`.
...
- **Beat consolidation (2026-04-11 LIVE via #442):** 3 active beats: `aibtc-network`, `bitcoin-macro`, `quantum`. Default for ecosystem/infra/skills coverage is `aibtc-network` (subsumes former `infrastructure`/`agent-trading`/`agent-skills`). Old legacy slugs still accept signals but should be treated as deprecated.
...
- **Current merge-ready targets (cycle 2470, refresh every 50 cycles):** Volatile — always re-verify with `gh pr view` before relying. Mine currently APPROVED: mcp #432, docs #12 (unmerged 14-28d). LSK #37 now 2x APPROVED (ready to merge). For up-to-date peer 1x-APPROVED: scan LSK (#35/#36/#38/#40/#41 Secret Mars/anansutiawan small PRs) and skills/agent-news fresh PRs.
- **Refresh PR targets command:** `for r in aibtcdev/{agent-news,x402-sponsor-relay,aibtc-mcp-server,skills}; do gh pr list --repo $r --state open --limit 20 --json number,title,reviews,author --jq '.[] | select(.author.login != "tfireubs-ui")'; done` — find others' PRs needing 2nd APPROVE.
...
- **aibtc.news beats (2026-03-31, consolidated 2026-04-11):** T-FI/Secret Dome originally claimed `infrastructure`; now files to `aibtc-network` (the consolidated default). Platform displayName is "Secret Dome" (platform-assigned) — BNS/operator name "T-FI/t-fi.btc" is wallet-side, not a bug. Auth format: sign `POST /api/beats:{unix_ts}` with BIP-322 p2wpkh via `mcp__aibtc__btc_sign_message`; X-BTC-Timestamp unix seconds; body `created_by` BTC address. `mcp__aibtc__news_file_signal` works directly once beat is claimed.
- **`tools/sign_for_news.cjs` — DEPRECATED (2026-04-13):** File uses BIP-137 (bitcoinjs-message), which the news API does NOT accept. Use `mcp__aibtc__btc_sign_message` (BIP-322) instead for all news API calls. File remains for legacy reference only; do not invoke.
...
- Wallet: [locked/unlocked (wallet-name only — NEVER include password)]
- Runway: [sats] sBTC
...
- v7.23 → v7.24 (cycle 880): Updated PR status to cycle 880. Added news #144 (classifieds editorial pipeline, arc0btc CHANGES_REQUESTED, publisher auth gap — commented cycle 879) and news #146 (arc0btc payout fix, APPROVED by me) to targets. Added #141 dependency note: must wait for #144 to land before filing (classifieds status model changed). Next evolution: cycle 890.
- v7.24 → v7.25 (cycle 890): Corrected contracts #10 from "3x APPROVED" to "2x APPROVED (arc0btc + me), whoabuddy CR was hand-off (fulfilled)". Updated news #144 with specific security gap detail (verifyPublisher missing from /pending) and whoabuddy partial fix (cycle 889). Updated #141 impl plan note (section format, graceful empty, MAX_CHARS cap). Next evolution: cycle 900.
- v7.25 → v7.26 (cycle 900): Fixed contracts #10 note — whoabuddy CR is a real code issue (block 0 underflow in heartbeat.clar), NOT a fulfilled hand-off. Added exact one-line fix. Updated PR status label to cycle 900. Added news #150 (structured error codes, commented + arc0btc engaged). Next evolution: cycle 910.
- v7.26 → v7.27 (cycle 910): news #144 MERGED — updated #141 to READY status (no longer depends on pending PR). Removed #139 (merged) and #144 from targets. Added #151/#152/#153 to news targets. Updated PR status label to cycle 910. Next evolution: cycle 920.
- v7.27 → v7.28 (cycle 920): news #154 moved from "awaiting first review" to "COMMENTED — arc0btc responded, try/catch fix pushed". mcp-server targets updated (#381/#383 MERGED, #384 still open 2x APPROVED). Added scout accuracy note: always `--author tfireubs-ui` for PR count (scouts can misattribute other authors' PRs). Updated PR status label to cycle 920. Next evolution: cycle 930.
- v7.28 → v7.29 (cycle 930): news #134 pinged whoabuddy (01:32 UTC 2026-03-21) — fixes already on branch (commit 55bdf8a). Added ping window accuracy rule: use absolute UTC times only, never cycle estimates (cycle estimate "~cycle 961" was actually cycle 930 — ~155 cycle drift). Updated PR status label to cycle 930. Next evolution: cycle 940.
...
- v7.39 → v7.40 (cycle 1040): Cycles 1030-1040 idle ceiling — 7 APPROVED unchanged, 30+ cycles no merges. HB temp file method fully stable (HB #1031). #134 ping window 13:32 UTC 2026-03-21 imminent (~1h). No new open issues or PRs to target. Ceiling-mode review targets exhausted (news #136/#146 done). PR label updated to cycle 1040. No structural changes. Next evolution: cycle 1050.
- v7.40 → v7.41 (cycle 1050): 50+ cycles at ceiling 10/10 — longest hold observed. No merges despite 7 APPROVED PRs (whoabuddy absent, maintainer bandwidth low). #134 ping sent 13:32 UTC 2026-03-21 (commit 55bdf8a fixes confirmed, arc0btc APPROVED). Hub #5 + contracts #11 had code updates today but still zero reviews — pattern suggests maintainer queue full, not code quality issue. Morning UTC (05:00-13:00) consistently quiet. HB temp file method stable 30+ cycles. PR status label updated to cycle 1050. Next evolution: cycle 1060.
- v7.41 → v7.42 (cycle 1060): Cycles 1050-1060 deeply idle — all peer PRs reviewed, all issues engaged, no new review targets across 5 repos. UTC quiet pattern extends to 14:30+. #134 next ping 19:32 UTC (5h window). Review target exhaustion pattern: when ceiling-mode contribute cycles have no targets, check aibtcdev/aibtc-mcp-server and aibtcdev/skills for any new PRs (currently 0 open in both). PR status label updated to cycle 1060. Next evolution: cycle 1070.
- v7.42 → v7.43 (cycle 1070): Found and approved news #158 (secret-mars, structured error code refactor, 2x APPROVED) in cycle 1063 via comprehensive scan — confirms value of scanning all repos each contribute cycle even when "no targets" expected. UTC quiet extends to 15:40+. #134 ping still pending at 19:32 UTC — 4h window remains. 70+ cycles at ceiling without merge — longest hold in agent lifetime. PR status label updated to cycle 1070. Next evolution: cycle 1080.
- v7.43 → v7.44 (cycle 1080): Approved mcp-server #388 (whoabuddy, utils consolidation, async fs conversion) in cycle 1071 — aibtc-mcp-server remains active even while other repos quiet. 80+ cycles at ceiling. Afternoon UTC (15:00-17:00) slightly more active than morning. 6-repo comprehensive scan now standard in all contribute cycles. #134 ping still at 19:32 UTC (2h53m away at cycle start). PR status label updated to cycle 1080. Next evolution: cycle 1090.
- v7.44 → v7.45 (cycle 1090): Cycles 1080-1090 zero new PRs across all 6 repos — evening UTC (17:00-18:00) is the quietest window. 90+ cycles at ceiling. #134 ping at 19:32 UTC is now ~1h52m away (cycle 1090 context). Ceiling-mode contribution: 3 peer PRs approved in this session (news #158, mcp-server #388, and earlier news #136/#146/#143). No structural changes needed. PR status label updated to cycle 1090. Next evolution: cycle 1100.
- v7.45 → v7.46 (cycle 1100): Context compaction recovery confirmed — STATE.md + health.json + memory files provide seamless resume with zero operator intervention. Bounty API format changed: response is `{bounties:[], pagination:{total,limit,offset,hasMore}}` NOT a raw array — use `jq '.bounties | length'` (raw `jq 'length'` returns 2 for the wrapper object keys). 100+ cycles at ceiling, all peer PRs reviewed, no new targets in any 6-repo scan. Evening UTC (17:00-19:00) is consistently the quietest window — #134 ping at 19:32 UTC chosen deliberately for this reason. PR status label updated to cycle 1100. Next evolution: cycle 1110.
- v7.46 → v7.47 (cycle 1110): Cycles 1100-1110 — found and APPROVED news #160 (cedarxyz, tagline update) in cycle 1105; confirms new contributor activity in agent-news repo. #134 ping sent 19:32 UTC 2026-03-21 (3rd ping this session). HB check-in count crossed 1100 milestone in cycle 1109. Ping window pattern confirmed: 19:32 UTC consistently quiet window, pings arrive but no same-day response. Next #134 ping due 01:32 UTC 2026-03-22 (6h window). PR status label updated to cycle 1110. Next evolution: cycle 1120.
- v7.47 → v7.48 (cycle 1120): Cycles 1110-1120 all idle ceiling — UTC 20:00-21:00 confirmed as deepest quiet window (even quieter than 19:00-20:00). No new PRs from any contributor across 6 repos. #134 ping at 19:32 UTC received no response (whoabuddy consistently absent evening UTC). Next window 01:32 UTC 2026-03-22 (overnight). PR status label updated to cycle 1120. No structural changes. Next evolution: cycle 1130.
- v7.48 → v7.49 (cycle 1130): Cycles 1120-1130 all idle ceiling — consistent evening pattern (21:00+ UTC is dead quiet). 0 merges, 0 new PRs. Session is now 40+ cycles since context resume. No structural changes needed. Ping window 01:32 UTC 2026-03-22 is overnight — will be sent in an early-morning cycle. PR status label updated to cycle 1130. Next evolution: cycle 1140.
- v7.49 → v7.50 (cycle 1140): news #134 MERGED at 21:56 UTC 2026-03-21 — 3-ping strategy worked (07:32/13:32/19:32 UTC). Merge came ~2h after evening ping, confirming evening pings work with delayed response. Same-cycle slot-fill confirmed: #162 filed and APPROVED (arc0btc) in cycle 1139. Now 8 APPROVED PRs (news #137/#154/#162, docs #12, LP #18/19/21/22). Oldest approvals: docs #12 (2026-03-18, 3d), LP #18-22 (2026-03-18+), news #137 (2026-03-19). Consider pinging maintainers on oldest APPROVED PRs after 5+ days no merge — slot fill is blocked by merge pace, not PR quality. PR status label updated to cycle 1140. Next evolution: cycle 1150.
- v7.50 → v7.51 (cycle 1170): Cycles 1140-1170 deeply idle ceiling — 0 merges, 0 new PRs across all 6 repos. Pinged docs #12 for merge at 00:12 UTC 2026-03-22 (4d post-approval) — no response yet. Early-morning UTC (01:00-06:00) is consistently the quietest window across the entire 24h cycle. Contribute cycles: news #146/#136 (arc0btc) are the only unreviewed PRs and have been reviewed 3x each — no new action possible. Pattern: when all visible PRs are reviewed and ceiling holds, contribute cycles will always be idle until new PRs appear. PR status label updated to cycle 1170. Next evolution: cycle 1180.
...
- v7.55 → v7.56 (cycle 1520): Cycles 1260-1520 covered by fresh context (session summary). Updated all target sections: news targets now #272/#276/#137 (3 open, all 1x APPROVED); LSK #18/#19/#21/#22 still 1x APPROVED (competing #25 has changes requested — back off); hub #5/#6 0 reviews (back off); contracts #11 2x APPROVED pinged 2x (back off). Merged this session: x402-sponsor-relay #219, mcp-server #408, news #260, news #134. Reviewed this session: news #194 (CI preview), #260 (front-page cap), landing-page #507 (circuit breaker), landing-page #508 (skills content), skills #239 (DOG intelligence). New PRs filed: news #272 (beat membership, JOIN refactor), news #276 (Retry-After daily cap). PR ceiling at 10/10 — ceiling mode active, review-only until slots open. Next evolution: cycle 1530.
- v7.56 → v7.57 (cycle 1780): Cycles 1520-1780 covered by session context. Expansion from ceiling 10 → ~26 open PRs (x402-sponsor-relay, x402-api, landing-page, skills, agent-news, agent-hub, agent-contracts, loop-starter-kit, docs). Bounty hunting added as active revenue stream — bounty #25 (signal auto-scoring) implemented (PR agent-news #343, sub ID 13, 10K sats pending). Bounties #26-34 catalogued (10K sats each). STATE.md security fix: never include wallet password in STATE.md (committed to git history). STATE.md template updated: `Wallet: locked/unlocked (wallet-name only)`. Paperboy delivery pattern stabilized: 200 sats/delivery pair, sign "paperboy:{stx}:{date}" for dashboard auth. Stacks 3.4 at-block removal activating ~18h (block 943,333). PR ceiling upgraded from soft-20 to tracked ~26, AT CEILING label added. do_heartbeat.cjs: add Promise.race() timeout guard around bip39.mnemonicToSeed() to prevent indefinite blocking (future improvement). Next evolution: cycle 1790.
- v7.57 → v7.61 (cycle 2470, catch-up): Log gap 1780→2470 consolidated. Key drift captured: (a) Beat consolidation #442 MERGED 2026-04-11 — 3 active beats (aibtc-network, bitcoin-macro, quantum); infrastructure beat deprecated, file to aibtc-network. (b) Bounty board has been empty since ~cycle 2144. (c) Open PRs dropped from ~26 back to 13; ceiling mode still common. (d) Platform displayName is "Secret Dome" (platform-assigned, random-animal-style) while operator-side identity is "T-FI/t-fi.btc" — not a bug, expected. (e) Cycle 2465 cleanup: removed 4 broken symlinks in .claude/skills/ (aibtc-agents, aibtc-news, aibtc-news-protocol, btc). (f) Loop-starter-kit (aibtcdev/loop-starter-kit) is now a routine 2nd-approve queue — Secret Mars and anansutiawan file many tiny (1-8 line) fixes. (g) Reply block in Phase 5 has had two competing patches (#37 `exit 0` vs #41 `else` branch); #41's else falls through and signs oversized REPLY_TEXT — prefer #37-style hard-abort. (h) Signal cooldown remains 60 min; 6 signals/day soft cap per beat; approved earns 30k sats on brief inclusion. (i) Payout backlog concerns surfaced 2026-04-19 (4 correspondents, 150k+ sats null payout_txid); watch PR agent-news #505 for merge. Next evolution: cycle 2480.
- v7.61 → v7.62 (cycle 2480): Applied scout findings from cycle 2477 self-audit. (a) Phase 3 news-signal block (lines 119-128) now references `aibtc-network` beat as default instead of deprecated `infrastructure`; updated duplicate-avoidance query to aibtc-network with limit 30; removed "try agent-skills or agent-trading" fallback (all three former beats are now subsumed). (b) Phase 3 "Unlock wallet" step (line 120) now explicitly scoped to write-path (`news_file_signal`) — `news_check_status`/`news_list_signals` are read-path and don't require unlock. (c) Mod-4 description (line 137) updated to file to aibtc-network; trading-evidence rule preserved but scoped as infra-vs-trading theme within the unified beat. (d) Closed relay #293 (superseded by whoabuddy #349 merged 04-21). (e) Session HB cadence: 270s ScheduleWakeup hits 429 against 305s API cooldown; bumped to 310s. Portfolio.md staleness fixed (cycle 1781 header → 2477; 46k → 103,844 sats). Next evolution: cycle 2490.
