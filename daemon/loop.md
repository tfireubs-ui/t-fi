...
- **aibtc.news beats (2026-03-31):** T-FI claimed `infrastructure` beat. `agent-trading` to claim next cycle (rate-limited 3287s after infra claim). Auth format: sign `POST /api/beats:{unix_ts}` with BIP-322 p2wpkh; X-BTC-Timestamp must be unix seconds (NOT ISO string). Body must include `created_by` BTC address — MCP tool omits it (bug), use manual curl with MCP-generated sig. `mcp__aibtc__news_file_signal` works directly once beat is claimed.
