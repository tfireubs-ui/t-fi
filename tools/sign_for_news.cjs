const bip39 = require("bip39");
const { BIP32Factory } = require("bip32");
const ecc = require("tiny-secp256k1");
const bitcoinMessage = require("bitcoinjs-message");
const bitcoin = require("bitcoinjs-lib");
const fs = require("fs");
const path = require("path");

const bip32 = BIP32Factory(ecc);

let MNEMONIC = "";
try {
  const envPath = path.join(process.env.HOME || "/home/claude-user", ".env");
  const envContent = fs.readFileSync(envPath, "utf8");
  const match = envContent.match(/^BTC_MNEMONIC=(.+)$/m) || envContent.match(/^MNEMONIC=(.+)$/m);
  if (match) MNEMONIC = match[1].trim();
} catch (e) {}

async function main() {
  const message = process.argv[2];
  if (!message) { console.error("Usage: node sign_for_news.cjs <message>"); process.exit(1); }
  const seed = await bip39.mnemonicToSeed(MNEMONIC);
  const root = bip32.fromSeed(seed);
  const btcChild = root.derivePath("m/84'/0'/0'/0/0");
  const { address: btcAddress } = bitcoin.payments.p2wpkh({ pubkey: btcChild.publicKey });
  const timestamp = new Date().toISOString().replace(/\.\d{3}Z$/, '.000Z');
  const sig = bitcoinMessage.sign(message, btcChild.privateKey, true, { segwitType: "p2wpkh" });
  console.log(JSON.stringify({ signature: sig.toString("base64"), timestamp, btcAddress }));
}
main().catch(e => { console.error(e.message); process.exit(1); });
